...
def generate_koords(n,m):
    return [(i,j) for i in range(0,n) for j in range(0,m)]

def generate_special_fields_koords(special_fields):
    special_fields_list = []
    for special_field in special_fields:
        special_field = special_field.split(",")
        special_fields_list.append(tuple([int(i) for i in special_field]))    
    return special_fields_list

def generate_special_fields_number(special_fields):
    return dict((special_fields[i], i) for i in range(0,len(special_fields)))
    
def generate_distance(spec_field_numbers,koords):
    results = []
    for koord in koords:
        distances_for_koord = dict((key,math.sqrt(math.pow((koord[0]-key[0]),2)+(math.pow((koord[1]-key[1]),2)))) for key,value in spec_field_numbers.items())
